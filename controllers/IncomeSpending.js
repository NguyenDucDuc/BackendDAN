...
const incomeSpendingController = {
    ...
    countSpending: async (req, res) => {
        try {
            const [[countSpendingDay]] = await sequelize.query(`select count(*) as 'countSpendingDay' from incomespendings where user_id=${req.data.id} and Date(createdAt) = Date(Now()) and type='SPENDING' `)
